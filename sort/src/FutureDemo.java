...
public class FutureDemo {
	public static void main(String[] args) throws InterruptedException, ExecutionException{
		...
		FutureTask<Integer> findMaxInSecondPart = 
                              new FutureTask<Integer>(new FindMax(a,1500,1999));
		FutureTask<Integer> findMidInSecondPart = 
                new FutureTask<Integer>(new FindMax(a,1000,1499));
		
		new Thread(findMaxInFirstPart).start();
		new Thread(findMaxInSecondPart).start();
		new Thread(findMidInSecondPart).start();
		
		int maxInFirst =  (int) findMaxInFirstPart.get();
		int maxInSecond = (int) findMaxInSecondPart.get();
		int midInSecond = (int) findMidInSecondPart.get();
		System.out.println("Max is " + 
                            (maxInFirst > maxInSecond ? maxInFirst:maxInSecond));
		System.out.println(midInSecond);
		//验证结果是否正确
		int max = a[0];
		for(int i = 0; i < 2000; i++){
			if(a[i] > max){
